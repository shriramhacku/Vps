...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED. BUY ACCESS FROM --> @DYNAMO_022*", parse_mode='Markdown')
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started by @Dynamo_022 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration} seconds*", parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    btn7 = KeyboardButton("Price List📄")
...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/JARVIS_HACKS/11*", parse_mode='Markdown')
        ...
        bot.reply_to(message, "*Join --> @Jarvis_Hacks*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.reply_to(message, "*DM --> @Dynamo_022*", parse_mode='Markdown')
    elif message.text == "Price List📄": 
        bot.reply_to(message, '''*🚀 New Plans Of DDoS :- 🚀
...
💥1 MONTH - ₹ 1200    
💥LIFETIME - ₹ 2500   
💥 All PLANS HAVE 600+ SEC POWER ✅
💥 24 Hours Bot Online

Dm Only :-
@Dynamo_022 ✅*""", parse_mode='Markdown') 
